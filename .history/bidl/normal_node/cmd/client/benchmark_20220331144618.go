...
// GenerateSortWorkload generate transactions for quicksort
// len: length of quicksort array
// @author wenlinli
func GenerateSortWorkload(len int, num int, org int) []*common.Transaction {
	txns := make([]*common.Transaction, num)
	dummy := make([]byte, 800)
	rand.Read(dummy)
	log.Infof("the sortarray length is %d, transaction number is %d, orgs is %d", len, num, org)
	for i := 0; i < num; i++ {
		org1 := rand.Intn(org)
		//org2 := org1
		payloadStr := "sortlength" + ":" + strconv.Itoa(len)
		orgStr := strconv.Itoa(org1)+ ":" + strconv.Itoa(org1+1)
		//orgStr := strconv.Itoa(i%2)
		sig := util.CreateMAC([]byte(payloadStr), []byte(common.SecretKey))
		txn := &common.Transaction {
			 Type:      common.TxnMessage,
			 Payload:   []byte(payloadStr),
