...
type Processor struct {
	Persists   map[uint64]int            // persist num
...
//quicksort
func quicksort(slice []int, left int, right int) {    //@author wenlinli
	i := left
	j := right
	if i == j {
		return
...
func (p *Processor) ExecuteTxn(txn *common.SequencedTransaction) {
	...
	nd := false
	if related {
		if payload[0] == "sortlength"{ // excute CPUHeacy workload @author wenlinli
			len, _ := strconv.Atoi(payload[1])
			env = p.ExecSortTxn(len, txn)
		} else if len(payload) == 2 { // account creation transaction
...
func (p *Processor) ExecSortTxn(length int, txn *common.SequencedTransaction) common.Envelop {
	//@author wenlinli
	log.Debugf("Starting quicksort workload, length of quicksort array: %d", length)
	nd := false
	slice := make([]int, length+1, length+1)
	for i := 0; i < len(slice); i++ {
		slice[i] = length - i
	}
	quicksort(slice, 0, length)
	log.Infof("After sort the array, length of array is %d, max element of array is %d .\n", length, slice[length])
	// generate wset by the greatest element
	wset := map[int]int{
		length : slice[length],
	} 
...
func (p *Processor) ProcessPersist(data []byte) {
	...
	if _, ok := p.Persists[result.Seq]; ok {
		p.Persists[result.Seq] = p.Persists[result.Seq] + 1
	} else {
		p.Persists[result.Seq] = 1
	}
	if p.Persists[result.Seq] == 3 { // 3f+1 = 4
		if _, ok := p.Envelops[result.TxnHash]; ok {
			env := p.Envelops[result.TxnHash]
			if env.SeqTransaction.HasCommited && !env.SeqTransaction.HasPersisted {
				...
				//p.Persists[result.TxnHash] = 0
				log.Infof("Transaction persisted, has commited, the transaction seqNum is %d, delete Envelop data! it hascommited value is %s , it haspersisted value is %s. The transaction end_to_end_time is %.3f ms.", env.SeqTransaction.Seq, strconv.FormatBool(env.SeqTransaction.HasCommited), strconv.FormatBool(env.SeqTransaction.HasPersisted), float32(end_to_end_time/1e6))
				delete(p.Envelops, result.TxnHash)
			} else {
				env.SeqTransaction.HasPersisted = true
				//log.Infof("Transaction Persisted, but not commit, the transaction seqNum is %d, it hascommited value is %s, it haspersisted value is %s", env.SeqTransaction.Seq, strconv.FormatBool(env.SeqTransaction.HasCommited), strconv.FormatBool(env.SeqTransaction.HasPersisted))
		    }
		}
	}
}
