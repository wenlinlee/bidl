...
for i in `seq 0 0`; do
    docker run --name normal_node$i --net=host --cap-add NET_ADMIN normal_node:latest /normal_node/server --quiet --tps=$2 --id=$i >  $base_dir/logs/normal_${i}.log 2>&1 &
done
...
cd $normal_node_dir
if [ $3 == "test" ]; then
    docker run --name bidl_client --net=host --cap-add NET_ADMIN normal_node /normal_node/client --num=1000000 --org=1 --tps=50
