...
func (p *Processor) ProcessBlock(block []byte) {
	...
	log.Infof("Commit latency: %d ms for block %d.tx number %d\n", elapsed, p.blkNum,hashNum)
}
...
func (p *Processor) ProcessPersist(data []byte) {
	...
	if p.Persists[result.TxnHash] == 3 { // 3f+1 = 4
		if _, ok := p.Envelops[result.TxnHash]; ok {
			...
			//log.Infof("The transaction seqNum is %d, it hascommited value is %s, it haspersisted value is %s", env.SeqTransaction.Seq, strconv.FormatBool(env.SeqTransaction.HasCommited), strconv.FormatBool(env.SeqTransaction.HasPersisted))
			if env.SeqTransaction.HasCommited == true {
				...
				log.Infof("Transaction persisted and had commited, the transaction seqNum is %d, end_to_end_time is %.3f ms.", env.SeqTransaction.Seq, float32(end_to_end_time/1e6))
				delete(p.Envelops, result.TxnHash)
			} else {
				p.Envelops[result.TxnHash].SeqTransaction.HasPersisted = true
                                env.SeqTransaction.HasPersisted = true
				//log.Infof("Transaction Persisted but not commit, the transaction seqNum is %d, ", env.SeqTransaction.Seq)
			}
		}
	}
}
