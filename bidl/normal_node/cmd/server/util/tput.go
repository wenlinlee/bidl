	...
	for {
		...
		case <-monitor.TputBlk:
			...
			if num % interval == 0 {
				// duration := int(time.Since(start).Nanoseconds())
				duration := int(time.Since(start).Milliseconds())
                                duration1 := int(time.Since(begin).Milliseconds())
				if duration == 0 {
					...
					log.Infof("BIDL block commit throughput: %f kTxns/s, avg throughput is %f kTxns/s\n", float64(interval*blkSize)/float64(duration),float64(num*blkSize)/float64(duration1))
				}
				start = time.Now()
				//num = 0
			}
